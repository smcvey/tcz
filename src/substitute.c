                   ...
                   case '%':
				       ...
				       strcat_limits(&str_data,Reflexive(sender,UPPER));
                                  ...
                                  case '@':  /* ---->  Miscellaneous substitution  <---- */
                                       ...
                                       break;
                                  case '$':  /* ---->  Line is a title, which should be underlined with 'set underline on'  <---- */
				       if(!(command_type & NESTED_SUBSTITUTION)) {
					  if(Validchar(player) && Underline(player)) {
                                       ...
                                       break;
                                  case '~':  /* ---->  '~~~~' underline, which should be ignored with 'set underline on'  <---- */
                                       if(((Validchar(player) && Underline(player))) && !(command_type & NESTED_SUBSTITUTION))
